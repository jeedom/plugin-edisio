...
Exemple : les sondes de temperature envoient leur données (signal, batterie et température par exemple) toutes les 45 secondes environ, même s'il n'y a pas eu de changement de valeur. +
...
Voilà à quoi ressemble la page du plugin Edisio (ici avec déjà 3 équipements) : 
...
* Ne pas vérifier la batterie : indique à Jeedom de ne pas vous alerter si l'équipement envoit une trame de batterie faible (certains modules ne gèrent pas correctement cette info et génèrent de fausses alertes)
