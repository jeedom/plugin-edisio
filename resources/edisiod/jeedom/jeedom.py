...
class jeedom_serial():
    ...
    def open(self):
        if self.device:
            logging.info("Open serial port on device: %s, rate %s, timeout: %i",
                         self.device, self.rate, self.timeout)
        else:
    ...
    def write(self, data):
        logging.info("Write data to serial port: %s",
...
class jeedom_socket_handler(StreamRequestHandler):
    def handle(self):
        ...
        logging.info("Client disconnected from [%s:%d]",
                     self.client_address[0], self.client_address[1])
