...
class _TimerReset(Thread):
    ...
    def run(self):
        logger.debug("Time: %s - timer running..." % time.asctime())

        while self.resetted:
            logger.debug("Time: %s - timer waiting for timeout in %.2f..." % (time.asctime(), self.interval))
            self.resetted = False
        ...
        self.finished.set()
        logger.debug("Time: %s - timer finished!" % time.asctime())

    def reset(self, interval=None):
        """ Reset the timer """

        if interval:
            logger.debug("Time: %s - timer resetting to %.2f..." % (time.asctime(), interval))
            self.interval = interval
        else:
            logger.debug("Time: %s - timer resetting..." % time.asctime())
...
def decodePacket(message):
	...
	if CMD in ['07'] and MID in ['01']:
		if  DimOff_threads.has_key(str(PID)+str(BID)):
			if clean_message == prevMessage and unixtime_utc_check < (timerDatetime+datetime.timedelta(milliseconds=800)) :
				logger.debug("Too soon to refresh timer")
			else:
				logger.debug("Thread exists resetting")
				timerDatetime = unixtime_utc_check
				dimmingOff_thread = DimOff_threads.get(str(PID)+str(BID))
				dimmingOff_thread.reset(1)
		else:
			logger.debug("Thread not exists creating")
			...
			timerDatetime = unixtime_utc_check
			dimmingOff.start()
		if clean_message == prevMessage and unixtime_utc_check < (prevDatetime+datetime.timedelta(milliseconds=4000)) :
			#logger.debug("Dimming in progress, ignore")
			return
	else:
		if clean_message == prevMessage and unixtime_utc_check < (prevDatetime+datetime.timedelta(milliseconds=200)) :
...
if __name__ == '__main__':
	...
	current_sensor_data = {}
	prevMessage = '';
	prevDatetime = datetime.datetime.utcnow();
	timerDatetime = datetime.datetime.utcnow();
